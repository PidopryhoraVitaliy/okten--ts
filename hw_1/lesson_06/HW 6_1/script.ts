{
    ...
    let cards1: CardType[] = cards.filter(card => card.cardSuit === 'spade' && card.value === 'ace');
    let cards2: CardType[] = cards.filter(card => card.value === '6');
    let cards3: CardType[] = cards.filter(card => card.color === 'red');
    let cards4: CardType[] = cards.filter(card => card.cardSuit === 'diamond');
    let excludedCardValues: string[] = ['6', '7', '8'];
    let cards5: CardType[] = cards.filter(card => card.cardSuit === 'spade' && !excludedCardValues.includes(card.value));
